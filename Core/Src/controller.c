...
PoseError_t* PoseError(Pose_t* ref_pose, Pose_t* curr_pose, bool ignore_map){
    ...
    error.L_err = ref_pose->coord%100 - curr_pose->coord%100; 

    return &coord_error; 
}

void SetupMaps(){
    for(uint8_t i = 0; i < GRID_SIZE; i++){ // y, traverses rows 
       for(uint8_t j = 0; j < GRID_SIZE; j++){ // x, traverses col
           MAP[1][i][j] = i*100 + j; 
           MAP[2][i][j] = (GRID_SIZE - 1 - i)*100 + j;
           MAP[3][i][j] = (GRID_SIZE - 1 - i)*100 + (GRID_SIZE - 1 - j);
           MAP[0][i][j] = i*100 + (GRID_SIZE - 1 - j);
        } 
    }
}

void PrintMaps(){
    for (uint8_t k = 0; k < NUM_MAPS; k++) {	
        for (uint8_t i = 0; i < GRID_SIZE; i++) { // y, traverses rows 
	        for (uint8_t j = 0; j < GRID_SIZE; j++){// x, traverses col
